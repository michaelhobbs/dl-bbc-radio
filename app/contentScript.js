...
const getDlButtonContainer = (linkTag) =>
    linkTag.querySelector('.sc-o-island')?.parentElement ||
    linkTag.querySelector('.sc-o-island') ||
    linkTag;

const programLinks = document.querySelectorAll(
    'a[href^="/sounds/play/"][data-bbc-result]'
);
...
for (var i = 0; i < programLinks.length; i++) {
    ...
    var x = new MutationObserver(function (e) {
        if (e[0].removedNodes) {
            extendProgramLink(
                e[0].target.closest('a[href^="/sounds/play/"][data-bbc-result]')
            );
        }
...
if (programUrlRegExp.test(window.location.pathname)) {
    ...
    const playerContainerTag = document.querySelector(
        '.play-c-herospace-container'
    );
