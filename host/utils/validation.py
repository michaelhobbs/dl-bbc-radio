...
def _validate_args(message, command):
    ...
    if 'args' not in command:
        ...
        for arg in command['args']:
            args_valid = args_valid and arg in message and re.match(
                command['args'][arg], str(message[arg]))
        return args_valid
...
def validate_message(message):
    ...
    '''
    isValid = 'cmd' in message and message['cmd'] in COMMAND_MAP and _validate_args(
        message, COMMAND_MAP[message['cmd']])
    if not isValid:
